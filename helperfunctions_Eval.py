...
def plot_comparison(original, mask1, mask_name,IMG_PATH,savepath):
    #import numpy as np
    ...
    print('saving combined image and mask')
    head, tail = os.path.split(IMG_PATH)
    savepath = savepath+tail
    try:
        all_images = [
            original, white_line,
            mask1, white_line
        ]
        image_combined = np.concatenate(all_images, axis=1)
        imsave(savepath, image_combined)
    except:
        print("An exception occurred")
def animal_count(Image,MASK,class_names,IMG_PATH,cl_image_size,Classifier,savepath):
    ...
    warthog = 0
    zebra = 0
    # convert the mask to gray scale before applying the otsu method
